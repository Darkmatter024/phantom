...
## 3 · Verify debt — ⏳ **ONE at `.585`** (2026-09-09). ⛔ This heading read *"ZERO at `.557`"* for 24 versions; the debt below `.557` is the historical record

⏳ **OPEN: `.585`.** RESTORE HONESTY (Batch 2 P0 sub-ship 2), on `main` (`3edf586`), pushed, unpromoted,
unstamped. **Owed, in order:** `tools/promote.ps1` → the one look (header ⋯ → RESTORE → pick any PHANTOM
backup from Files → the confirm reads `Replaces on this device:` with `N on this device → M in backup`
lines and ends `This will OVERWRITE current data.` → **Cancel**, nothing changes) → `tools/verify.ps1 585
PASS` or `FAIL "reason"`. Evidence `docs/BATCH2-585-EVIDENCE.md`. The write set is unchanged by this ship;
what the phone proves is that a ten-line `confirm()` and the Files picker read cleanly on iOS.
...
✅ **2026-09-09, against `v1.14.584` (served, `d02a081`).** Two looks on the backup banner: header ⋯ →
...
⏳ **PARKED ON `.585` (`3edf586`), PUSHED TO `main`, UNPROMOTED AND UNADJUDICATED (2026-09-09).**
...
with `verify.ps1 585 PASS|FAIL`. ⭐ **THE PROMOTE ORDER IS RULED (`OWNER-RULINGS.md` 2026-09-09 PROMOTE
