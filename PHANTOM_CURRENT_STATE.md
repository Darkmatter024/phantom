...
## 3 · Verify debt — ⏳ **ONE at `.584`** (2026-09-09). ⛔ This heading read *"ZERO at `.557`"* for 24 versions; the debt below `.557` is the historical record

⏳ **OPEN: `.584`.** BACKUP HONESTY (Batch 2 P0), on `main` (`6439dbc`), pushed, unpromoted, unstamped.
**Owed, in order:** `tools/promote.ps1` → the one device sequence (header ⋯ → BACKUP → dismiss the
sheet → the magenta `BACKUP NOT SAVED — share sheet dismissed — tap to retry` strip stays and no
download prompt follows → tap the strip → Save to Files → the strip clears and a toast reads
`Backup exported HH:MM`) → `tools/verify.ps1 584 PASS` or `FAIL "reason"`. Evidence
`docs/BATCH2-584-EVIDENCE.md`. Playwright cannot see the real iOS share sheet; the two looks are what
the phone is for.
...
✅ **2026-09-09, against `v1.14.583` (served, `b9d7de9`).** SYS reports `.583`; BUILD's execution strip
...
⏳ **PARKED ON `.584` (`6439dbc`), PUSHED TO `main`, UNPROMOTED AND UNADJUDICATED (2026-09-09).**
BACKUP HONESTY — Batch 2 P0, owner GO 2026-09-09 (*"GO on .584 Batch 2 P0"*), the first ship built
under the one-command gate. **Verify debt is ONE and the gate is AT its limit** — no `.585` by hook
(`phantom-guard.js`) or by `promote.ps1` Guard 4 until John promotes `.584`, runs the two-look phone
sequence in `docs/BATCH2-584-EVIDENCE.md`, and adjudicates with `verify.ps1 584 PASS|FAIL`. *(`.583`
closed 2026-09-09: device PASS, stamped `b9d7de9`, first live `verify.ps1 583 PASS`.)*
