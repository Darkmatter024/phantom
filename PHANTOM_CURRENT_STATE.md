...
## 3 · Verify debt — ✅ **ZERO at `.584`** (2026-09-09). ⛔ This heading read *"ZERO at `.557`"* for 24 versions; the debt below `.557` is the historical record
...
✅ **`.584` CLOSED 2026-09-09 — device PASS on both looks, stamped `d02a081`, promoted, `SERVED` (owner, `verify.ps1 584 PASS`).**
BACKUP HONESTY — Batch 2 P0, owner GO 2026-09-09 (*"GO on .584 Batch 2 P0"*), the first ship built and
adjudicated end to end under the one-command gate. **Verify debt is ZERO.** A `.585` is possible by hook
(`phantom-guard.js`) and by `promote.ps1` Guard 4; **it needs an owner GO — the next candidate is queue
item 1b, restore honesty.** *(`.583` closed 2026-09-09 the same way: device PASS, stamped `b9d7de9`.)*
...
1b. ⏳ **Batch 2 P0 sub-ship 2 — RESTORE honesty (rev 2 §1) — NEEDS ITS OWN GO; `.584` is adjudicated, one word starts `.585`:** show what will be replaced, validate the backup before applying, report partial or failed recovery honestly — the import side of the same surface (`phantomImport`, the `_bs` schema guard). `.584`'s bundle now carries `partial:true` + `readFailures` and `manifest[].included:false`, which is exactly what a validating restore reads. Not folded into `.584`: a different visible surface, one visible change per ship.
