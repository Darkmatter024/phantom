...
**Last updated: 2026-09-21, after `v1.14.594` (A.2 Ship 3) was pushed to `main`.
⏳ **VERIFY DEBT IS ONE (2026-09-21): `.594` (A.2 SHIP 3 — THE NOTES ADAPTER) IS ON `main`, NOT STAMPED.** `VERIFIED` and `release` stay at `.593`. The fourth adapter reads `phantom_deploy_audit_v1` through `_rr_readKey` and emits `note.logged` for `RACK_NOTE` entries matched three ways on `action`, `entityType` and `entityId` (ruling Q-18); a reset marker anywhere in the log is reported in the notes coverage row, which now carries a `detail` on an `empty` row too (Q-23), never as a count (lead L-7). `status` gains no key (Q-22). ⚠ **Gate engine `laptop-chromium` again** (owner ruling 2026-09-17): WebKit was probed first and is still blocked by Smart App Control. **Ship 4 probes again and returns to the owner.** **Proof:** RED on `.593` was 14 failed / 13 passed, each red for the missing feature, re-proved against untouched `c24cf12` in a worktree; GREEN is spec `65` **27/27** first time — Ship 3's Phase 0 counts were all correct, no repeat of Ship 2's D-1; all 12 planned mutations caught, each restore proved byte-exact. Regression, each spec alone: `64` 8/8 (report output unchanged), `00-boot` 4/4, `63` 5/5, `12` 8/8, `13` 13/13, `27` 8/8, `04-storage` 18p/2f/1s, `98` 26p/1f, `01-nav` 12p/6s/1f — every red reproduced identically on untouched `c24cf12`. `phantom-ship-gate` PASS; `phantom-rd-reviewer` PASS with one advisory (a stale date line, fixed). The parent session re-ran `65` 27/27, `64` 8/8 and `04-storage` itself. **Evidence** `docs/A2-SHIP3-EVIDENCE.md`; Phase 0 `docs/A2-SHIP3-PHASE0-EVIDENCE.md` (Q-18…Q-27 ruled as recommended). The device look is evidence §6, in a Safari tab on staging, and **for this ship it is the only WebKit gate.** **Next is John's:** the §6 look → `.\tools\verify.ps1 594 PASS` or `FAIL "…"` → `.\tools\promote.ps1`. **Then Ship 4** (photos adapter + removal of the `?rrdev=1` readout) ends the A.2 slice.
⛔ **SUITE HEALTH, FOUND WHILE BUILDING, NOT THIS SHIP'S DOING: two `04-storage` tests are now red on the PROMOTED, VERIFIED `.593`.** `:461` fails `ReferenceError: PHANTOM_BACKUP_EXCLUDED_KEYS is not defined` and `:605` fails `locator.click: Element is not visible` on `#pe-tapcatch`; both passed on 2026-09-17 and both fail identically on untouched `c24cf12`, so the environment moved, not the code. The same `#pe-tapcatch` signature produced a one-off `01-nav:399` failure that did not reproduce in four further runs. The box also slowed about 3× mid-build. **Owner's to schedule: the cause is not established, and nothing was changed on account of it.**
✅ **ZERO UNVERIFIED SHIPS (2026-09-17) — TRUE AS OF `.593`, SUPERSEDED BY THE ROW ABOVE: `.593` VERIFIED AND PROMOTED BY THE OWNER.** The owner ran `verify.ps1 593 PASS` (stamp `d73a0bb`, pushed) and then `promote.ps1` (`release` fast-forwarded `ac827cc → d73a0bb`), both from the session prompt with the `!` prefix. Claude Code ran neither. `VERIFIED` = `main` = `release` = served `.593`. Before this line was written, that was checked against GitHub (`git ls-remote`: `main` = `release` = `d73a0bb`) and against the served Pages bytes: `version.json`, `sw.js` and `dct-ios.html` all carry `phantom-v1.14.593`, and the served `dct-ios.html` hashes the same as `release`'s blob. The promote also published the docs commits `1168926`, `472ba8e` and `d48fc0c`. ⚠ **Two earlier reports of this verify and promote had not landed:** first the stamp, then the promote. Each time `origin` and Pages had not moved and this clone's reflog showed nothing, and each command worked when rerun from the session prompt. The cause was not established. The D-1 shape again: each claim was checked against `origin` before it was written down. **Next:** A.2 Ship 3 is not started. It tries `phone-webkit` first; the Chromium gate does not carry forward. **Still open, owner's to schedule, NOT A.2 work:** Q-10, Q-15, Q-16 (rows below).
✅ **`.593` BUILD RECORD — WAS VERIFY DEBT, CLEARED BY THE ROW ABOVE: `.593` (A.2 SHIP 2 — BLOCKERS ADAPTER, DERIVED `status.openBlockers`, THE Q-A `gaps` LIST) WAS PUSHED TO `main` AS `f2f2cec`.** ⚠ **Gate engine `laptop-chromium`, not `phone-webkit`** (owner ruling 2026-09-17, `OWNER-RULINGS.md`): Windows Smart App Control blocks Playwright's WebKit on this box and a reinstall did not help. The ruling does not carry forward. **Proof:** RED on `.592` was 11 failed / 7 passed, each red for the missing feature. GREEN is spec `65` **18/18**, and seven mutations were each caught. Regression, each spec run alone: `64` 8/8, `00-boot` 4/4, `63` 5/5, `12-blockers` 8/8, `13-phase-model` 13/13, `04-storage` 20+1 skipped. `98-cmd-census` and `01-nav` each fail one phone-only check at 1366px, and the same test fails on untouched `d48fc0c`. `phantom-ship-gate` PASS 7/7; `phantom-rd-reviewer` PASS. graphify was not run: Application Control blocks it. **Evidence** is `docs/A2-SHIP2-EVIDENCE.md`. Its §6 is the device look, in a Safari tab on staging (ruling Q-8), and **for this ship it is the only WebKit gate.** **Next is John's:** the §6 look → `.\tools\verify.ps1 593 PASS` or `FAIL "…"` → `.\tools\promote.ps1`. Ship 3 waits for the stamp (one at a time) and tries `phone-webkit` first. **Still open, owner's to schedule, NOT A.2 work:** Q-10, Q-15, Q-16 (row below).
✅ **ZERO UNVERIFIED SHIPS (2026-09-16) — TRUE AS OF `.592`, SUPERSEDED BY THE ROW ABOVE: `.592` VERIFIED AND PROMOTED BY THE OWNER** — the owner ran `verify.ps1 592 PASS` (stamp `ac827cc`, pushed) and then `promote.ps1` (`release` fast-forwarded `526d907 → ac827cc`), both from the session prompt with the `!` prefix; Claude Code ran neither. `VERIFIED` = `main` = `release` = served `.592`, confirmed before this line was written from `origin` (`origin/main` = `origin/release` = `ac827cc`) and from the served bytes on BOTH Pages and staging (`version.json`, `sw.js` and `dct-ios.html` all carry `phantom-v1.14.592`, and the served app contains `PHANTOM_RR`). The promote also published two docs-only commits, `2007296` and `dc2d00c`: the September dossier and the standalone FORGE concept demo, at `darkmatter024.github.io/phantom/docs/PHANTOM-SEPTEMBER-DOSSIER.html` and `…/docs/phantom-forge.html`, both hash-matched to `release`. ⚠ **One earlier report of this verify and promote had not landed** — `origin` and Pages stood still and nothing had run in this clone; the cause was not established. A second clone on this box, `nexus01-ruling-0915`, is checked out on branch `ruling/automated-promote`, where `verify.ps1` refuses (`BRANCH`). The D-1 shape again: the claim was checked against `origin` before it was written down. ⚠ **Found in passing, not acted on:** that clone holds an unpushed commit `0c30096` "ruling: automated stamp and promote re-granted behind script-enforced gates (2026-09-15)". It is not on `origin` and not in force; `CLAUDE.md` on `main` still says promote is owner-only. **Next:** A.2 Ship 2 (blockers adapter + the Q-A `gaps` list) is being built on `.592`. Its Phase 0 is `docs/A2-SHIP2-PHASE0-EVIDENCE.md`, and **Q-10 … Q-17 were ruled as recommended** on 2026-09-17. ✅ **Q-A was ruled 2026-09-17: a separate `gaps: [{ field, detail }]` list**, so coverage stays one row per adapter. Ships 2–4 are verified **one at a time** (`OWNER-RULINGS.md` 2026-09-17). ⚠ **Open defects found by that Phase 0, owner's to schedule, NOT A.2 work:** UNBLOCK never closes the blocker record (Q-10); the phase-card BLOCK button writes a blocked phase with no record (Q-15); and "blockers" names three different counts (Q-16). **Was:** ⏳ VERIFY DEBT WAS ONE: `.592` (A.2 SHIP 1 — RACK RECORD ASSEMBLER CORE + `?rrdev=1` READOUT) ON `main`, NOT STAMPED. Cherry-picked from `f0466da` onto the verified `.591` (identical patch-id); spec `65` 10/10; regression on `.591` code `64` 8/8, `00-boot` 4/4, `63` 5/5, `98-cmd-census` 27/27, `04-storage` 20+1 skipped; `phantom-ship-gate` PASS; evidence `docs/A2-SHIP1-EVIDENCE.md`, device look in a Safari tab on staging (ruling Q-8).
✅ **ZERO UNVERIFIED SHIPS (2026-09-15, 13:45) — TRUE AS OF `.591`, SUPERSEDED BY THE ROW ABOVE: `.591` VERIFIED AND PROMOTED BY THE OWNER** — stamp `526d907` at 13:25:42, `release` fast-forwarded at 13:29:01; `VERIFIED` = `main` = `release` = served `.591`, confirmed from `origin` and from the served bytes on BOTH Pages and staging (all three stamps plus the fix line) before this line was written. ⚠ **Two earlier reports of the same verify and promote had not landed** — origin, Pages and this box's PowerShell history all stood still until the third run; recorded because it is the D-1 shape again, a claim checked against origin before it was written down. **Was:** ⏳ VERIFY DEBT WAS ONE: `.591` ON `main`, NOT STAMPED. ✅ `.590` was verified and promoted by the owner on 2026-09-15 (stamp `fd1fa40`; `main` = `release` = served `.590` when `.591` was cut). ⭐ **`.591` = FORGE DETAIL PAGER: PREV/NEXT WALK THE ROW THE HEADER COUNTS.** Owner report on staging: rack `c1:005` read `ROW C1 · POS 5/18` with NEXT dimmed. Root cause: POS read the row (`rowOf()` → `ROWS[row]`) while PREV/NEXT read the five-rack 3D bench (`LOADOUT`, capped by `setLoadout`'s `.slice(0, 5)`, Contract A6); `c1:005` was the last bench slot. Now both read the row, an off-bench neighbour moves the bench along the row (still five live racks), a walked bench is never saved over the hand-built loadout, and the picker opens on the saved one. Ruling `OWNER-RULINGS.md` 2026-09-15 (supersedes the unrecorded 2026-09-14 "label the counts"); evidence `docs/RACK-PAGER-NEXT-PHASE0-EVIDENCE.md` §10; pinned by `test/e2e/66-forge-pager-row-walk.spec.js`. ⏳ **A.2 SHIP 1 IS NEXT, BY OWNER RULING:** `a2/ship1-assembler` @ `f0466da` was cut from `.590` and must be re-based onto `main` before it takes `.592`, which the guard allows only after `.591` is adjudicated.
✅ **RESOLVED 2026-09-15 — `.590` VERIFIED AND PROMOTED BY THE OWNER (stamp `fd1fa40`). The row below is kept as the record of how it stood:** ⛔ **VERIFY DEBT IS ONE: `.590` IS ON `main`, SERVED BY STAGING, AND NOT STAMPED.** Measured 2026-09-14 from seven independent channels, every one of which says the same thing: working-tree `VERIFIED` line 1 = `phantom-v1.14.589` · local `HEAD` = `5ecdb3c` (the ship commit, no stamp commit after it) · local `release` = `99cc1e4` · `origin/main` = `5ecdb3c` · `origin/release` = `99cc1e4` (live `git ls-remote`, not a cached ref) · served release `version.json` = `.589` (cache-busted) · exactly one `verify.ps1` exists on the box, in this clone. ⭐ **The PHANTOM icon is on `.589`; PHANTOM STAGING is on `.590`.** ⛔ **THIS BLOCKS THE NEXT SHIP MECHANICALLY, AND THAT IS THE GATE WORKING:** `phantom-guard.js` `checkVerifiedGate` compares `HEAD:version.json` against `VERIFIED`'s first token and refuses any `version.json` commit while they differ, so **no `.591` can exist until `.590` is adjudicated.** Docs-only and test-only commits are unaffected.
⚠ **THE OWNER REPORTED A PASS AND A PROMOTE THREE TIMES AND ORIGIN NEVER MOVED — RECORDED BECAUSE THIS IS D-1's SHAPE, NOT BECAUSE ANYONE MISREMEMBERED.** The reading order held: origin → Pages → then the report, and a claim does not outrank a mismatch. ⭐ **The diagnosis that came out of it is worth more than the incident.** Round 1 refused `NOT-SERVED` because his clone was behind the push (`HEAD` ≠ `origin/main`). Round 2 refused `NOT-SERVED` again after a pull — and the cause was **neither of the two exits anyone reads that guard as having**: `tools/verify.ps1:255` runs its **own `git fetch`** and refuses as `NOT-SERVED` when it fails, so a **fetch problem and a staging problem print the same guard name**. His fetch was exiting **128** while the identical command in the identical clone exited 0 from both Git Bash and PowerShell in another session — a per-session environment fault, cleared by opening a fresh window. ⛔ **AND THE REFUSAL CANNOT SAY WHICH:** `Invoke-Git` (`:84`) throws `"git fetch --quiet origin failed with exit code 128"` and **discards git's stderr**, so the one guard whose job is to explain prints an exit code and nothing else. **A tooling defect, logged, not fixed** — it needs an owner GO like anything else.
✅ **ZERO UNVERIFIED SHIPS (2026-09-13) — TRUE AS OF `.589`, SUPERSEDED BY THE ROW ABOVE.** `VERIFIED` = `main` = `release` = served = **`.589`** (`99cc1e4`), confirmed from `origin` AND from the served bytes at `darkmatter024.github.io/phantom` (3,637,685 bytes, `FEATURE_IMAGE_INPUT` reads `true` in the served file) before this row was written. Both promotes this session were the owner’s, from his terminal, in the canonical order **verify → stamp → promote**; Claude Code ran neither `verify.ps1` nor `promote.ps1` at any point.
⭐ **`.589` IS IMAGE INPUT FOR THE DCT ASSISTANT** (`4350d34` ship, stamp `99cc1e4`) — a technician attaches a photo to a question and gets an answer. Five phases of `phantom-image-input-plan.md` in one version: characterization tripwire, compression, flag+branch, storage guard, send-time budget + the flag ON. **The existing `.txt/.log/.json/.eml/.csv` import is unchanged and that is PROVED, not asserted** — the image branch sits at the top of `va_importTicket` and returns, nothing below it was edited, and with the flag switched OFF the `accept` attribute is byte-identical to what it was before the feature existed (`58-assistant-import-file.spec.js` asserts exactly that). ⛔ **THE CEILING IS THE DESIGN, NOT THE PIXEL SIZE:** `phantomAPI` refuses a body over `PHANTOM_API_MAX_BODY` (300,000) BEFORE the network and base64 inflates 4/3, so the plan’s stated 1600px@0.80 → "under 400 KB" target was **533 KB base64, nearly double a limit that already existed** — and the app’s own pre-existing 1280/0.70 default produced 271,259 bytes from a noisy 3000×2000 JPEG, over the ceiling on its own. `va_prepareImage` walks a ladder and takes the first rung that fits a budget **measured at send time from the real request envelope**, because the system prompt also carries `buildContext`, `brief_buildLiveContext`, `hwMatrix_queryContext` and a ticket capped at 12,000 chars. ⭐ **ONE COMPRESSOR, NOT TWO:** the plan asked for a new `compressImage()`; `photo_compress` (rack photo capture) already did that work, so it was **parameterised** — Contract A2 — with defaults reproducing the old hard-coded 1280/0.70 so its single zero-argument caller is served byte-identically. That function had **zero test coverage** before this ship; its default behaviour is now pinned. ⭐ **PHASE 4 BUILT NO STORE, AND THAT IS THE FINDING:** its premise was "conversation state lives in localStorage", and **the assistant persists no conversation at all** — `VA` is in-memory and dies with the sheet — so there was nothing to divert and a store would have been a THIRD photo database beside `phantom-attachments` and `phantom-photos`. It shipped the GUARD instead: tests that go red the day a photo reaches localStorage. Pinned by `58`–`62` (27 tests). ⛔ **NOT IN `.589`:** Phase 6 camera capture (one attribute, deliberately last) and alpha flattening (deferred by owner ruling — a transparent PNG still returns as JPEG).
⚠ **`.588`’S BYTES ARE NOT WHAT ITS OWN RELEASE NOTES DESCRIBE, AND A READER MUST NOT RECONSTRUCT THE SHIP FROM THEM.** `version.json` at `.588` describes INTEL-DOCK Ship 1 only, because that is what it was when the number was cut (`ded1f6b`). **Two further app fixes landed on `main` under the same stamp** before it was ever adjudicated — `7ef2729` update coordination and `10d8d2c` navigation — because the guard correctly refuses a `version.json` bump while the previous version is unadjudicated, and `.588` could not be adjudicated as "the dock change" any more. The owner verified and promoted `.588` as the three changes together on 2026-09-11 (stamp `4a9893f`). ⭐ **The lesson was acted on, not just recorded:** when the same drift threatened `.589`, the owner chose **stamp-first** — `.588` adjudicated on staging bytes that provably did NOT contain image input (verified by fetching the served file and confirming `FEATURE_IMAGE_INPUT` absent), and only then was `.589` cut. One stamp, one meaning.
⭐ **`.588`’S TWO FIXES, BOTH FIXED BY DELETION RATHER THAN BY ANOTHER GUARD.** (1) **UPDATE COORDINATION:** `dct-ios.html` carried THREE independent reactions to one signal — two parse-time IIFEs (one `location.reload(true)` on version mismatch, one `reg.unregister()` then reload) plus `phantom_versionFileBackstop()`, which does the correct thing. The two IIFEs bypassed the `_swUpdating` gate `v1.14.458` built on purpose, raced each other, and **looped**: measured against a routed mismatch the app could not finish booting at all. The unregister twin was the dangerous one — it destroys the registration, so the reload behind it has no controller and no cached shell, turning a working offline app on marginal signal into one that cannot boot. Both DELETED; nothing replaces them. (2) **NAVIGATION:** `showPage()` cleared every `.page` and `.tn-item` BEFORE looking for `#pg-<id>`, and the `v1.14.548` recovery branch it leaned on selected `pg-cmd` or **`pg-triage`** — a page `v1.14.555` had deleted. The lookup is hoisted above the sweep; a missing destination now warns, toasts and returns, leaving the technician where they were. ⚠ **REACHABILITY STATED PLAINLY:** `redesign_isOn()` is a hard `return true` since Stage 7a, so the `.181` guard intercepts every id outside `cmd/work/ref/master` and all four exist — **the inverted ordering was NOT reachable and no blank screen could be produced.** A latent-defect fix, kept because LEGACY-RETIRE deletes a page per ship and the trap arms itself the moment a whitelisted id loses its page. Pinned by `56-update-coordination.spec.js` (9) and `57-nav-recovery.spec.js` (8).
⛔ **RACK POSE IS FIXED AND DELIBERATELY PARKED — OWNER RULING 2026-09-13: DO NOT MERGE.** `fix/rack-pose-determinism-v2` @ **`955608a`**, two commits, NOT on `main` and not to be merged without a new GO. ⭐ **AND THE DIAGNOSIS CORRECTS THE BASELINE’S OWN FRAMING: IT IS NOT TEST ISOLATION AND NOTHING LEAKS STATE BETWEEN SPECS.** `_ease` converged a flat **0.14 PER FRAME** and set an axis exactly only under `LOCK_SNAP` (0.0009), so a walk-exit gap needed ~56 frames — under a second at 60fps, **~21 seconds** in a harness rendering at 2.7fps. The pose was **APPROACHED, not INSTALLED**, and its RESTING value depended on the frame budget; earlier specs starve that budget, which is the entire reason `37-locked-rack-pose` looked order-dependent while passing 4/4 alone. It rested at `-4.69565` against a canonical `-4.7`, violating `.455`. **The fix:** ease factor from elapsed SECONDS (`k = 1 - (1-0.14)^(dt*60)`) plus a 1.6s deadline (`LOCK_EASE_MAX`) that SETS the pose however few frames arrived. ⛔ Raising `LOCK_SNAP` was rejected as the dishonest option — it shrinks the residual while leaving the frame dependence, so the spec would go green while the property stayed broken. Owner ruling 2026-09-10 stands: *"it’s a real defect, the tolerance stands."* **EVIDENCE, measured on `4350d34` not inherited from the older branch:** `54-pose-frame-starvation.spec.js` **5/5 RED → 5/5 GREEN**, residuals `fwd.x 0.00162 @ 80 frames` and `0.00139 @ 80 frames` before, and each run fell 1.1m → ~30s after. Spec 37 is 4/4 but that is **corroboration, not proof** — isolation passed before the fix too. ⏳ **STILL OWED IF IT IS EVER UNPARKED:** the full single-process suite run, which is CI’s job; this laptop OOM-killed a 38-spec prefix attempt, confirming `docs/TEST-SUITE-RECOVERY-PLAN.md` again rather than disproving it.
⭐ **THE HARNESS CAN NOW EXECUTE THE APP’S OWN SERVICE-WORKER PATH, AND COULD NOT BEFORE.** `dct-ios.html` gates `register()` on `location.protocol === 'https:'`, which is narrower than the platform rule — 127.0.0.1 IS a secure context but the app never asks. **Measured on the old harness with the app booted, on every project: `registrations: 0, controller: false, PHANTOM_SW_REG: false`.** Registration, `updatefound`, the version backstop and the whole update-coordination path therefore ran in NO test, which is how two ungated reloaders survived 130 versions under a green 430-test suite; `05-offline.spec.js` had already filed it as harness truth #1 / a P2. `test/server.js` now serves the same handler on http (unchanged, still default) AND https at `PORT+1` with a self-signed cert generated on demand into gitignored `test/.certs/`, and a **`sw-https-chromium`** project points at it. ⚠ **SCOPED ON PURPOSE:** the five original projects are untouched, so the pinned Windows baseline stays comparable row-for-row. `.github/workflows/e2e-sw-https.yml` runs that project on chromium in ~2 minutes, gated by `test/ci-assert-ran.js`, which reads the JSON reporter’s `status` field so that skipped / absent / ambiguous / failed / report-missing are ALL failures. ⛔ **That gate was wrong TWICE before it was right, and both cuts are instructive:** the first accepted a SKIPPED test because a skipped test still prints its own title; the second demanded the literal marker `ok`, which Playwright prints only on win32 (`✓` everywhere else), so it passed on the laptop and failed on Linux while every test passed. **A gate that exists to make a vacuous green impossible must not itself depend on how a reporter draws a tick.**
⭐ **FULL-SERIAL CI IS GREEN-BY-NAME AT `.589`** — run #5 on `4350d34`: **480 passed, 4 failed, and the four are the pinned baseline names** (`10-site-profile-root:84` stale test, `37-locked-rack-pose:141` and `:179` order-dependent, `39-sw-update-path:64` flake). `docs/PLAYWRIGHT-BASELINE.md`’s own rule applies: *"`4 failed` matching this list is clean; `4 failed` with a different member is a regression wearing the right number."* ⚠ **AND A FIRST READING OF THAT RUN WAS WRONG, RECORDED SO IT IS NOT REPEATED:** CI’s previous run had ZERO failures, so four was read as a regression caused by adding five canvas-heavy spec files to a suite that exists on its own machine because it barely fits in memory. The NAMES refuted it, and so did the clock — run #5 covered 52 more tests in 41m against run #4’s 47m, so the suite is not straining. ⛔ **`37:141` and `:179` will keep appearing in every full run until the parked pose fix is unparked; they are KNOWN AND DIAGNOSED, not new signal.**
✅ **ZERO UNVERIFIED SHIPS (2026-09-09).** `VERIFIED` = `main` = `release` = served = **`.585`** (`f95ece0`). ✅ **`.585` PASSED ON DEVICE 2026-09-09** — the RESTORE confirm read as designed (`Replaces on this device:` with per-section counts, ending `This will OVERWRITE current data.`), Cancel, nothing changed — and was adjudicated by **the owner with `verify.ps1 585 PASS`** (stamp `f95ece0` carrying `VERIFIED` alone, `release` level with `main`, `SERVED: phantom-v1.14.585`; confirmed from `origin` and the served stamps before this row was written). Ran under the **NAMED EXCEPTION** (`OWNER-RULINGS.md` 2026-09-09 PROMOTE ORDER): serve → see → verify one more time because no staging surface for `main` existed; **not precedent** — from here the canonical order is verify → stamp → promote, runnable once staging exists. ⚠ Two `verify.ps1 585 PASS` attempts before the promote were refused at NOT-SERVED and wrote nothing — the guard, not a fault. `.585` = RESTORE HONESTY (`3edf586`, Batch 2 P0 sub-ship 2; gate 7/7, review PASS, spec 52 8/8 RED→GREEN); evidence `docs/BATCH2-585-EVIDENCE.md`. ✅ **STAGING EXISTS AND THE TOOLS FOLLOW IT (2026-09-09)** — `main` is served at `https://phantom-staging.wfj6t2fk7w.workers.dev` (owner-created Cloudflare Worker with static assets; all five served files byte-identical to `origin/main`, measured from this box), and `f7dc691` moved `verify.ps1`'s NOT-SERVED / SERVED-BYTES to `origin/main` + staging, took the promote out of its PASS path, restored `promote.ps1` Guard 4 (the **incoming** version must be adjudicated, FAILED never promotes) and rewrote CLAUDE.md's loop to **build → check staging on device → verify → promote**; `verify-selftest.ps1` 10/10 in a throwaway clone. The canonical order is runnable from the next ship; `.585` stays the named exception's last run. ✅ `64e6cf8`: `wrangler.jsonc` pins staging to `html_handling: none` — `/dct-ios.html` answers 200 like Pages, `.git` not uploaded; the bare `/` is now 404 there, a lead, owner's call (§7). ✅ **`.584` PASSED ON DEVICE 2026-09-09, BOTH LOOKS** — dismissed sheet → the magenta `BACKUP NOT SAVED — share sheet dismissed — tap to retry` strip stays and no download prompt follows; retry → Save to Files → the strip clears and the toast names the time — and was adjudicated by **the owner with `verify.ps1 584 PASS`** (stamp `d02a081` carrying `VERIFIED` alone, promoted, `SERVED: phantom-v1.14.584`; confirmed from `origin` and the served `version.json` + `sw.js` before this row was written). `.584` = BACKUP HONESTY (`6439dbc`, DATA-HONESTY-COMMAND Batch 2 P0, owner GO 2026-09-09; `phantom-ship-gate` 7/7, `phantom-rd-reviewer` PASS after one red-vs-magenta correction, spec `51-backup-honesty` 6/6 RED→GREEN); evidence `docs/BATCH2-584-EVIDENCE.md`. **Nothing blocks a `.585` — the next ship needs an owner GO, not a gate (queue 1b, restore honesty).** ✅ **`.583` PASSED ON DEVICE 2026-09-09** — SYS reports `.583`, Build's strip reads `LOCAL ACTIVE | ON-DEVICE · SAVED` — and was adjudicated by **the owner, from his terminal, with the first live run of `tools/verify.ps1 583 PASS`** (stamp `b9d7de9` carrying `VERIFIED` alone, `release` `944ba1d → b9d7de9`, `SERVED: phantom-v1.14.583`; confirmed from `origin` and the served `version.json` + `sw.js` before this row was written). One keystroke where there were two commands and a push. ⚠ **THE SAME SESSION CAUGHT A FALSE PASS FIRST (D-1):** on 2026-09-08 a `.583` PASS was reported before any promote had run — `origin/release`, local `release`'s reflog, all three served stamps and this box's PowerShell history all still read `.582`, so nothing was stamped; the phone turned out to be on `.582`, whose strip cannot read SAVED. **A claim cannot outrank a mismatch, and the reading order is origin → Pages → then the report.** `.583` = STATUS-HONESTY, one string on Build's execution strip; evidence `docs/BATCH1-583-EVIDENCE.md`. **Nothing blocks a `.584` — the next ship needs an owner GO, not a gate.** *(Prior state of this row: `VERIFIED` = `.582` (`9a9d6db`); `.583` on `main` (`aa4d437`) awaited promote → device → verify.)* ✅ **`.582` PASSED ON DEVICE 2026-09-08** (*Active rack* card up, DIAGNOSTICS reads `PREVIEW/MOUNT_NOT_DRAWING` — candidate A on hardware, §8) and was stamped VERIFIED by **Claude Code running `tools/stamp.ps1` on the owner's explicit in-session instruction** after his reported pass — the doctrine's *"except when John says stamp it"* carve-out, recorded so the actor is credited (D-2); the owner's own stamp attempt earlier that hour had not written. `.582` was a DIAGNOSTICS-ONLY instrument (RACK-PREVIEW-CONTEXT Phase 1, Option 5 of `docs/RACK-PREVIEW-CONTEXT-PHASE0-EVIDENCE.md`, owner-ruled 2026-09-08); its harness precheck is green (`49-preview-trace.spec.js` **4/4** on `phone-webkit`) and the mechanical ship gate is PASS. ⛔ **THE VERIFY ORDER THIS ROW STATED UNTIL 2026-09-08 WAS WRONG — `main` WAS NEVER SERVED.** Pages was traced to `release` by content match and by a last-modified six minutes after `release`'s own commit. *"Device-verify against `main` → stamp → promote"* was therefore unexecutable, and it is how `.581` came to be stamped VERIFIED off a dry-run misread with no phone having seen it. **Owner ruling C, 2026-09-08 (`30f3822`): the honest order is PROMOTE → device-verify on the Pages URL → STAMP either way**, and `tools/promote.ps1` Guard 4 now adjudicates the version the phone HAS — promoting `.583` while `.582` sits unruled is still refused. ⭐ **`.581` DID REACH A PHONE ON 2026-09-08 AND STAYS VERIFIED:** BUILD's rack preview was blank after a Master load with a canvas present and nothing recorded; the owner ruled it the iOS-WebKit async GPU-grant class, pre-existing since `.391`, with every function in the preview chain byte-identical `.580`→`.581`. ⛔ **`.582` MUST BE ADJUDICATED BEFORE ANY `.583` EXISTS**, and the gate is mechanical, not a promise: `phantom-guard.js` `checkVerifiedGate` compares `HEAD:version.json` against `VERIFIED`'s first token and refuses any `version.json` commit while they differ. ✅ *(The refusal that held `.581`'s false `version.json` sentence in place was lifted by the `.581` stamp; `.582`'s bump carries the corrected note.)* ⭐ **THE `.581` SHIP COMMIT IS MIS-TITLED IN HISTORY AND A READER MUST NOT CONCLUDE `.581` NEVER SHIPPED:** `f32e49a` reads *"stamp: phantom-v1.14.580 VERIFIED"* but carries the whole `.581` ship — `dct-ios.html +36/−9`, `sw.js`, `version.json` — because the old `stamp.ps1` committed everything staged. **Fixed forward by `694ef3b`** (*"stamp.ps1 commits the register file and nothing else"*), which also proved the partial-commit technique in a throwaway repo; the misleading commit stands because rewriting history is forbidden. `git log --oneline -- version.json` is therefore not a reliable ship ledger — **these rows are.** ⭐ **FIRST-DOOR Ship A is COMPLETE AND FULLY ADJUDICATED** across `.572`–`.578` (`.572` FAILED and was repaired by `.573`; `.576`, `.577` and `.578` each passed on device). **Nothing blocks a `.579`** — the next ship needs only an owner GO, not a gate. ⚠ **AND THE GATE HAD TO ENFORCE THAT ONCE ALREADY:** A-4 was BUILT on `.576` while `.576` was still unadjudicated — the stacking condition the rule exists to prevent. Nothing stacked in the repo because the guard refused the commit, but **the flag should have come from the session before starting, not from the hook after finishing.** ⭐ **`VERIFIED` IS TWO STAMPS BEHIND IN GIT** (`.576` committed vs `.578` in tree) — **committing it is the owner's, from a terminal**: the guard refuses `VERIFIED` on the agent path by design. **The working-tree file is what the guard reads, so ships are not blocked; the committed HISTORY is what lags.**
...
## 3 · Verify debt — ⛔ **ONE: `.590`, shipped 2026-09-13, unstamped as of 2026-09-14.** See the header row. *(Was ✅ ZERO at `.585`, 2026-09-09.)* ⛔ This heading read *"ZERO at `.557`"* for 24 versions; the debt below `.557` is the historical record
...
⏳ **F-0a · Shift readiness still renders a PERCENT on the zero-state Command screen — owner saw `33%` with a ring, 2026-09-10.** Recorded from the `.586` staging look; **not investigated further and nothing changed.**
**What the source actually says (read at `.586`, not recalled):** `cs_renderReady` (`dct-ios.html:23644`) computes `var pct = answerable.length ? Math.round(met / answerable.length * 100) : null` and writes `#cs-ringn` as `(pct === null) ? '—' : (pct + '%')`. So `.577` did **not** delete the score. What `.577` added is the *fully*-unanswerable case: its own comment reads *"⛔ NO NUMBER AT ALL WHEN NOTHING IS ANSWERABLE. 0% would read as 'measured, and you are failing'"*, and the arc is gated on `pct > 0`. `33%` is therefore the intended output of one gate met out of three the app judged answerable — **the code is doing what `.577` shipped**, which makes this a question about the *ruling*, not a code regression.
⛔ **THE RULING CANNOT BE CHECKED FROM THIS REPO. `H-10` DOES NOT EXIST IN IT.** The owner's account is that harvest item **H-10** said keep the named gates *without* a percentage. `grep -rn "H-10"` across every `.md` in the repo returns nothing, and across `Downloads/*.md` returns nothing. **This is the `SHIP-TECH-FLOW-V2` lesson again — a ruling no session can see is not a programme of record.** Until H-10 is produced, the disposition is undecidable: if H-10 said drop the percentage, this is a *never-removed* item, not a regression; if it did not, the percentage is behaving as designed and the owner is re-opening it. **Owner to supply H-10 or restate the ruling.**
⚠ Two sub-questions that ride with it and are *not* answered here: whether a ratio over "answerable" gates is honest at all when the denominator itself moves (three today, four tomorrow), and whether a zero-state screen should show a ring before a Master exists.

⏳ **F-0b · RACKS cell counts a `0` with no Master loaded — the `.579` pattern, still live.** Owner confirmed it on the `.586` staging look, 2026-09-10. Already on the DATA-HONESTY-COMMAND pile (`docs/SHIP-HANDOFF-VERIFY-AND-CLOSEOUT-rev2.md` §1, *"racks cell counted `0` with no Master → `na`/em-dash per the `.579` pattern"*); this entry records that it **survived `.586` and was seen on hardware**, so it is a confirmed live defect rather than a lead. **Precedent for the fix is `.579`** (state `:131`): the blockers KPI stopped counting what it cannot know after the `.578` cold-launch FAIL, for exactly this reason — a counted `0` on a fresh device claims a measurement the app never made. ⛔ Not fixed here; it belongs to its Batch 2 ship, not to `.586` and not to INTEL-DOCK.
...
**Two served surfaces since 2026-09-09.**
- **`release` → GitHub Pages** (`darkmatter024.github.io/phantom/`), the **PHANTOM** icon. Graduated. ⛔ This line said `main` until 2026-09-08, when Pages was traced to `release` by content match and by a last-modified six minutes after `release`'s own commit (`30f3822`). Intermittent failure mode of record: `build` ✅ but `deploy` ❌ while githubstatus is all-green — a transient repo-side lock, **not code**. An empty commit re-triggers.
- **`main` → staging** at `https://phantom-staging.wfj6t2fk7w.workers.dev` — a Cloudflare **Worker with static assets** (workers.dev, not pages.dev), production branch `main`, preview branches none, owner-created 2026-09-09 per `docs/INFRA-STAGING-CLOUDFLARE-PAGES.md`; the **PHANTOM STAGING** icon, its own origin and its own storage. Rebuilds on every push to `main` in about a minute. `phantom-api` Worker CORS v2.3 allows the origin. Measured 2026-09-09: `version.json`, `sw.js`, `dct-ios.html`, `manifest.json`, `index.html` byte-identical to `origin/main`. ✅ **`64e6cf8` (owner GO 2026-09-09) pins the Worker from the repo: `wrangler.jsonc` with `html_handling: none`, `.assetsignore` (+ `.gitattributes` LF pin — wrangler splits it on bare LF, CRLF ignores nothing).** Measured after the rebuild: `/dct-ios.html`, `?legacy`, `/index.html` **200, no redirect**; `/.git/*` 404; seven served files byte-identical to `origin/main`; Pages untouched. ✅ **Owner ruled 2026-09-09:** the bare `/` stays **404** on staging — *"Skip the _redirects file — no verify path uses the root and I'd rather keep staging simple."* Closed, no file added. ✅ The PHANTOM STAGING icon was re-added from `/dct-ios.html` after the redirect fix and loads fine (owner, 2026-09-09).
...
⛔ **BEFORE ANYTHING ELSE: `.590` MUST BE ADJUDICATED.** It is on `main` (`5ecdb3c`), staging serves it, `release` is on `.589`, and `VERIFIED` still reads `.589`. **No further version can ship until it is ruled on** — the guard refuses a `version.json` bump while `HEAD` and `VERIFIED` disagree. The owner's step, from a **fresh** terminal in `nexus01`: `.\tools\verify.ps1 590 PASS` (or `FAIL "what you saw"`), then on PASS `.\tools\promote.ps1`. ⚠ **If it refuses `NOT-SERVED`, read the sentence under the banner before re-running** — that guard covers three different causes and one of them is a failing `git fetch` in that session, not a staging problem (header row). ⛔ Claude Code runs neither script, in any mode.

**The ONE look, still owed:** Command → readiness card, on a deployment with no handoff saved for it. **PASS** = `Handoff started` reads **OPEN** with a **gold** dot. **FAIL** = it reads **OK**, or the dot is **green**.

⏳ **A.2 HANDOFF RECEIVED AND IMPORTED, 2026-09-15 — GO'd 2026-09-15.** `docs/SHIP-HANDOFF-A2-ASSEMBLER.md` (7,133 bytes, `sha256 127da11f…a556`, byte-identical to the owner's copy). Ship 1 Phase 0 is `docs/A2-SHIP1-PHASE0-EVIDENCE.md`. ✅ **Q-1…Q-9 RULED AS RECOMMENDED, 2026-09-15** (`OWNER-RULINGS.md`, read that entry) — including **Q-1, which AMENDS Ruling 2: the assembler is built beside `deploy_generateReport` and does not touch it.** ⏳ **SHIP 1 IS BUILT AND HELD ON BRANCH `a2/ship1-assembler` @ `f0466da` — NOT on `main`, NO version.** Assembler core, registry, identity + phases adapters, and the `?rrdev=1` readout: `dct-ios.html` +225/−0 in one block, no pre-existing line touched. Spec `65` **10/10** on `phone-webkit` after RED on `.590`; six mutations each caught; `64` 8/8, `00-boot` 4/4, `63` 5/5, `98-cmd-census` 27/27, `04-storage` 20+1 skipped (its baseline), each run alone; `phantom-ship-gate` PASS 7/7, `phantom-rd-reviewer` PASS. Evidence `docs/A2-SHIP1-EVIDENCE.md` (on the branch). ⛔ **THE ONLY THING HOLDING IT IS `.590`:** once adjudicated, the branch merges to `main` with the three-stamp bump and the owner's loop runs. ✅ **The branch is PUSHED** — `origin/a2/ship1-assembler` @ `f0466da` (2026-09-15, owner instruction). Pushing it moves neither `main`, staging nor `release`. ⏳ **Q-A, before Ship 2:** keep the synthetic `rack` coverage row carrying Q-2's reason, or move gaps to their own list. ⭐ **The finding that shapes the build:** `safeGet` returns one fallback for absent, unreadable and malformed storage and WRITES a quarantine record on malformed, so no adapter built on it can tell `empty` from `error` or stay a pure read. ⭐ **A torture Master can reach a composite rack** through MASTER SCOPE → CREATE DEPLOYMENT, and that path stores the Master cab id as `rack.rackId`.

⭐ **`.590` = HANDOFF READINESS PREDICATE** (`5ecdb3c`, external review Finding 3/P1; evidence `docs/FIX-590-EVIDENCE.md`, Phase 0 `docs/HANDOFF-READINESS-PREDICATE-PHASE0-EVIDENCE.md`). The gate derived *"Handoff started"* from `!!(hraw && JSON.parse(hraw))` over the raw key, and **`[]` is parseable and truthy** — so an empty list, another deployment's handoff, and a handoff from months ago all read as a PASS. ⭐ **The label was wrong in the other direction too:** `handoff_generate` writes nothing, `handoff_saveRecord` is the only writer, so the key holds **completed** handoffs only, globally, forever. Pinned by `test/e2e/63-handoff-readiness-gate.spec.js` — **4 RED → GREEN on `phone-webkit`**; its fifth case passed pre-fix and is stated as a regression guard, not counted. ⚠ **Scope was deliberately narrowed and that is the judgement to review:** six surfaces read the shared `handoffDraft` boolean, and fixing it there would have made three of them *more* precisely wrong, so only the gate moved.

⚡ **TWO HARNESS FACTS MEASURED DURING `.590`, BOTH CONTRADICTING THE BOARD.** (1) **`phone-webkit` LAUNCHES AND RUNS CLEAN ON THIS BOX** — 81 tests across three specs, no launch failure. Board Q-5 says it cannot start here because Smart App Control blocks `webkit-2336`'s `libxslt.dll`, and that every local run is therefore Chromium signal only. **Stale — `.590`'s evidence is primary-project evidence.** Why the block lifted is not established. (2) ⛔ **`docs/PLAYWRIGHT-BASELINE.md` NO LONGER DESCRIBES THIS BOX FOR `04-storage`:** `:461`, `:605` and `:668` fail intermittently in multi-spec batches, **none of them baseline members**, all green standalone, and the failing set **moves between runs** — a control batch at `.589` failed two of them where the `.590` batch failed a different one. The same 49 tests took **1.5h** in one run and **5.5m** in another. That is the documented WebKit GPU climb, not code. **A re-pin is owed from CI, not from here.**
...
2. ✅ **RULED 2026-09-14 — rack identity AND the engine question, together.** Owner, verbatim: *"composite is the key, extend deploy_generateReport"* (`OWNER-RULINGS.md`, read that entry, not this line). The composite (`:32243`, the rack record's `id`) is the canonical rack key — already used by phases, blockers and photos, so `read(siteId, rackId)` is satisfiable. A.2 **extends** `deploy_generateReport`, Contract A2. ⛔ **Three things the ruling does NOT settle and that are each their own work:** the **Master bridge** (the composite comes from the vendor EDP parse, so no `s1:001` linkage is persisted — now a mapping problem, not an identity one); the **photo `siteId` defect** (`:57301` colon-splits a colon-free key; self-consistent, so a fix without a migration orphans every stored photo — Contract B11, its own ship); and **notes/audit still cannot be scoped to a rack by field** (2 of 24 writers pass `meta.rack`; the only contract-legal recovery is parsing the composite back out of `phase.id`, which embeds it at `:32258`). ⚠ **And the extension carries a compatibility constraint:** `deploy_generateReport` has three live consumers (JSON export, print HTML, close-out) reachable from four buttons, **no spec pins any of them**, and a characterization test over the current output is owed before the first extending edit.
3. ✅ **DONE 2026-09-14 — `docs/A1-CENSUS-REFRESH-TIER-A.md`**, the five Tier-A classes at `.590`, with eight measured corrections to A.1 (chief among them: audit writes `entityType`/`entityId`, not the census's `resource`/`resourceId`, so an adapter built to the census returns empty events forever). ⛔ **Twelve other A.1 classes are NOT refreshed** and any adapter for them still faces the original census. ⛔ **It did not go through `storage-archaeologist`:** that agent, `adapter-reviewer` and `report-fidelity-auditor` are files in the repo's `.claude/agents/` but are **not loadable agent types** in a Claude Code session — only the user-level agents are. CLAUDE.md's *"agents barred, equivalents run inline"* holds; a prior evidence doc of mine claimed otherwise from file existence and is corrected in the refresh's §0.
...
order are (1) RESTORE-UNDO — **Q2 ruled 2026-09-09: A, the header band** (`OWNER-RULINGS.md`); still waiting on **Q3's two numbers from the phone** (the backup file's size in Files; `N MB / M MB` from the STORAGE gauge on SYS → PROFILE — no new slice needed, `4ceaf21`) **and the owner's GO** — then (2) the rest of Batch 2. *(The
staging steps and the script-only follow-up this row used to gate on are done — see above.)*
