...
## Scope
1. **Snapshot before the first write** — every key in the restore's write set with its previous value, `phantom_ghost_echo_v1` before the merge, and the `GhostEchoDB` ghosts only when the file carries ghosts; one record in a new IndexedDB store, taken in one transaction, with provenance. **If it cannot be taken, the restore is refused with one line (ruled):** `Restore not started — could not save an undo point (<reason>). Nothing was changed.`
...
5. **Spec 54** — evidence §8, RED against `.585` first. (Spec 53 is the honest round-trip pin, already in the tree.)
