...
- Live: `v1.14.181` — **Ship B4 SHIPPED (John's explicit go 2026-07-04) — Ship B COMPLETE, awaiting the B4 device-verify (HARD STOP).** Prior: `.178`–`.180` and `.174`–`.177` device-verified by John, all good.
...
- Open decisions on John: **device-verify `.181` (B4 — the gating one; checklist in version.json)** · census SIGN-OFF (gates R1) · R-2.5 / F-1: define or confirm superseded · post-B4 data pass (TRIAGE/OPS INDEX rd homes) yes/no.
