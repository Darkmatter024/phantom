...
- Live: `v1.14.176` — **awaiting John device-verify (HARD STOP) of `.175` (PLATFORMS correction) + `.176` (Rack Map pill hide) + `.174` (PARTS labels).** Prior: .167–.172 device-verified (all good, cork cleared); **.173 landing REJECTED on device** (flat tiles) → replaced by `.175`.
...
- Queue: **Blast Radius re-home** (R-3b part 2; needs John's design call or a handoff spec — `renderBlastRadius` + 2 sub-renders hard-target the legacy `#br-wrap` id in 3 places, so a Deploy move is a real re-home of the node OR parametrizing the 3 fns to take a container; hiding its ref-strip pill without a Deploy door would STRAND it) → **Ship B** (router gating; John sign-off REQUIRED — also carries the deferred `deploy_showTools`/`deploy_showOpsTool` deletion + `nav_restore` `'tools'`/`'tool:'` cleanup) → **R1 deletion pass** (RETIRED incl. Crash-Cart Mode, after Ship B device-verified).
- R-3b `.176` done: hid the duplicate Rack Map pill (Work RACK MAP card is its canonical door). Blast Radius pill intentionally kept until re-homed.
