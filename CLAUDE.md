...
- Live: `v1.14.205` — **pt-btn-primary border-box->padding-box** (owner-requested follow-up to the .204 flag): the primary cask is a raw <button>, border-box ring did not reliably paint on iOS; swapped to the padding-box primitive (real cyan ring + padding-box glass + inset glow, --glow-border one-knob). Scoped to primary; ghost/secondary/qa-btn left as-is (device-verified rendering .190/.191/.198). Prior: **web-Claude FIX PASS .204** (adopted onto my .203 provenance-verified; fixed omni-bar, .ask double-rule + border-box invisibility, T3, D-1 stat-face title-crush).
