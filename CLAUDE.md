...
- Live: `v1.14.184` — **data pass (11 INDEX entries → honest rd routing: 5 TRIAGE→`ref:'rf-optics'` [content-parity verified: cards live in fs-triage, re-homed into rf-optics], parser→`rf-cli`, portmap→`tool:`, SOPs→`rf-know`, scan×2→`wk:'scan'`, BOM→`tool:'bom'`; 2-line rd-gated `tool:`/`wk:` hook mirroring `ref:`) — awaiting device verify (HARD STOP).** Prior: **`.181`–`.183` device-verified by John (2026-07-05, "all good") — SHIP B + B4b + B200 badge CONFIRMED ON HARDWARE**; `.174`–`.180` verified earlier.
...
- Open decisions on John: **device-verify `.181`–`.183` in one pass (checklist in version.json)** · census SIGN-OFF (gates R1) · R-2.5 / F-1: define or confirm superseded · **Icon Batch 2 art drop** (`ICON-BATCH-2-HANDOFF.md` in Downloads: 5 platform five-swap pieces + POWER/COOLING domain pair — NONE arrived yet; §2 B200 badge fix already shipped `.183`; §4 staged pieces also unidentified in Downloads) · `PHANTOM-PUNCH-LIST.md` referenced by two specs but doesn't exist — deliver or Claude creates it on the batch ship.
