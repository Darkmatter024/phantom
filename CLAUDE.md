...
- Live: `v1.14.206` — **A-2 assistant-sheet buttons border-box->padding-box** (completeness sweep off .204/.205): audited every glass <button>; the 5 A-2 buttons (.va-icard intent cards ×4, .va-cask/.va-ldone, .va-cback, .va-intake-chip) matched the .ask broken pattern (border-box, no appearance reset, last verified .194 pre-bug) — converted to the padding-box primitive. qa-btn/pt-btn-ghost/secondary/fr-go left (confirmed rendering). Prior: **pt-btn-primary .205**
