...
- Live: `v1.14.183` — **B200 badge fix (.183, AIR→DLC violet per owner ruling; Icon Batch 2 §2 pulled forward by "b200 fix now") on top of Ship B4 (.181) + B4b (.182) — all three stacked-unverified by owner instruction; ONE device pass covers `.181`–`.183` (HARD STOP, checklist in version.json).** Prior: `.174`–`.180` device-verified by John.
