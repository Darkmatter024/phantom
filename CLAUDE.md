...
- Live: `v1.14.177` — **awaiting John device-verify (HARD STOP) of `.174` (PARTS) + `.175` (PLATFORMS) + `.176` (Rack Map pill) + `.177` (Blast Radius → Deploy).** Prior: .167–.172 device-verified (all good); **.173 landing REJECTED** (flat tiles) → replaced by `.175`. R-3b now COMPLETE.
...
- Queue: **Ship B** (router gating; John sign-off REQUIRED — carries the deferred `deploy_showTools`/`deploy_showOpsTool` deletion + `nav_restore` `'tools'`/`'tool:'` cleanup) → **R1 deletion pass** (RETIRED incl. Crash-Cart Mode + the now-dead `rd_openPower`, after Ship B device-verified).
