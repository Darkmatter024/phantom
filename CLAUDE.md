...
- Live: `v1.14.204` — **web-Claude FIX PASS** (files49, adopted onto my .203; provenance-verified = my .203 + the diff exactly, CRLF-preserved). Corrects ON-DEVICE render bugs in my glass/faces work: #omni-bar opacity (text ghosting); .ask double-rule (my 8844 material was a silent cascade no-op); **.ask border-box invisible on raw <button> — my ask-card glass never painted on device** (owner confirmed twice), replaced w/ padding-box primitive; T3 primary-button tune; **D-1 stat-face layout crush — my .201/.203 faces squeezed card titles to a sliver** (fixed w/ card-hdr flex-wrap + full-basis stat/tags). ⚠OBSERVED, not acted on: pt-btn-primary is also a raw <button> w/ an un-stripped border-box bg — possible same invisibility, future device-verify finding. Prior: **D-1b .203 fleet stat faces**
