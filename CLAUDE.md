...
- Live: `v1.14.180` — **device-verified `.178`–`.180` by John (2026-07-04), all good. B1/B2/B3 router foundation confirmed; B4 awaits John's explicit go (shared `showPage` engine, highest blast radius).** Prior: `.174`–`.177` device-verified, R-3b COMPLETE.
...
- Queue: **Ship B** (router gating — plan drafted + adversarially verified in `Downloads/SHIP-B-PLAN.md`; 4 sub-ships): **B1 `.178` DONE** (repoint 15 deploy-flow `nav_push` `p:'sop'`→`p:'work'` under redesign; zero `nav_restore` edit) · **B2 `.179` DONE** (deleted `deploy_showTools`/`deploy_showOpsTool` + cleaned `nav_restore` `'tools'`/`'tool:'` — the `'tool:'` repoint changed BOTH the `typeof` condition and the call per the verify catch; kept `DEPLOY_TOOLS` + `_deploy_toolsBackHdr`) · **B3 `.180` DONE** (gated `showOpsTab`'s choke-point push `p:'sop'`→`p:'work'` under redesign; restore→`showPage('work')` paints no strip since `showPage` fires `showOpsTab` only for `id==='sop'`) → **B4 (HELD — needs John's explicit go + a prior device-verify)**: central **`showPage()`-level** guard (NOT `nav_restore`-level, per the verify catch — the Ref-search bar calls `showPage(legacy)` directly) + host-hardening + optional `showStab`. Highest blast radius (modifies shared `showPage`; `?legacy=1` byte-identical is the whole ballgame) — do NOT ship without a device pass on `.178`–`.180`. → **R1 deletion pass** (RETIRED incl. Crash-Cart Mode + dead `rd_openPower`, after Ship B verified).
