...
- Live: `v1.14.175` — **R-3b PLATFORMS correction just shipped; awaiting John device-verify (HARD STOP).** Prior: .167–.172 device-verified by John (all good, cork cleared); **.173 landing REJECTED on device** (flat tiles failed the design bar) → replaced by .175; .174 (PARTS labels) still pending verify.
...
- Queue: **R-3b pill cleanup** (deferred, transitional: drop Blast Radius + Rack Map pills from the `#rf-hw` pwr-sub strip; give Blast Radius a Deploy-detail entry — `renderBlastRadius` hard-targets `#br-wrap`) → **Ship B** (router gating; John sign-off REQUIRED — also carries the deferred `deploy_showTools`/`deploy_showOpsTool` deletion + `nav_restore` `'tools'`/`'tool:'` cleanup) → **R1 deletion pass** (RETIRED incl. Crash-Cart Mode, after Ship B device-verified).
